...
export async function testTalaModel(config: TalaModelConfig) {
  const reply = await request(config, [
    { role: "system", content: "You are TALA, the private operations agent for Azarraga Glass & Aluminum. Never invent business data." },
    { role: "user", content: "Connection test. Reply only: TALA model connected." },
  ], 60);
  return { ok: true as const, model: config.model, reply };
}

export async function runTalaText(config: TalaModelConfig, messages: Array<{ role: "system" | "user" | "assistant"; content: string }>) {
